...
"""
A mofified implementation of the algorithms described in the paper:
...
Original Programmers: Yoni Baruch, Yevgeny Ivanov and Daniel Isakov
Modifiying Programmers: Yevgeny Ivanov and Koren Abdush
Date: 22-06-25
"""
...
def remove_duplicates(matching):
    # Remove doubles
    grouped = defaultdict(list)
    for elem in matching:
        grouped[elem['spht_value']].append(elem)

    # Choose the best element per group
    filtered_matching = []
    for group in grouped.values():
        # Choose the element with the highest confidence
        best = max(group, key=lambda e: (e['confidence_1'], e['confidence_2']))
        filtered_matching.append(best)
    matching = filtered_matching
    # print("--------------------")
    # print("--------------------")
    # print("REMOVED DOUBLES:")
    
    return matching


def find_confidenceMean(matching):
    confidenceDiffTotal = 0
    matchingLen = len(matching)
    for element in matching:
        confidenceDiffTotal+= abs(element['confidence_1'] - element['confidence_2'])
        # print(f"confidenceDiffTotal adding: {abs(element['confidence_1'] - element['confidence_2'])} = {element['confidence_1']} - {element['confidence_2']}")
    
    # print(f"confidenceDiffTotal: {confidenceDiffTotal}")
    # print(f"confidenceDiffTotal /= matchingLen: {confidenceDiffTotal / matchingLen}")
    confidenceDiffTotal /= matchingLen
    return confidenceDiffTotal
...
if __name__ == "__main__":
    ...
    visualize_stars(img1 ,detected_stars_1, f"Detect/{img1.split("/")[1].split(".")[0]}-detect.png")


    detected_stars_2 = detect_stars(img2)
    visualize_stars(img2 ,detected_stars_2, f"Detect/{img2.split("/")[1].split(".")[0]}-detect.png")
    ...
    # random.seed(42)
    bsc = get_star_catalog()
    subset_bsc = random.sample(bsc, 5)  # Take a random subset of X elements
    ...
    # Execute online algorithm on the images
    result_1 = stars_identification(detected_stars_1, spht , al_parameter, camera_scaling_factor)
    result_2 = stars_identification(detected_stars_2, spht , al_parameter, camera_scaling_factor)
    matching = []

    # construct the new matching entries from both results
    for entry_1 in result_1:
        for entry_2 in result_2:
            if entry_1['spht_value'] == entry_2["spht_value"]:
                formatted_entry= {
                    "coords_1" : entry_1["coords"],
                    "coords_2" : entry_2["coords"],
                    "spht_value": entry_1["spht_value"],
                    "confidence_1": entry_1["confidence"],
                    "confidence_2": entry_2["confidence"]
                }
                matching.append(formatted_entry)
                # print(f"{entry_1['spht_value']}: {entry_1["coords"]} | {entry_2["coords"]}")
    ...
    # find confidence mean
    confidenceMean = find_confidenceMean(matching)

    # generate visual lines between stars:
    ...
    img2_resized = cv2.resize(image2, (new_w2, h1))

    vis = cv2.hconcat([image1.copy(), img2_resized.copy()])

    matchinLen = len(matching)
    for element in matching:
        pt1 = (int(element["coords_1"][0]), int(element["coords_1"][1]))
        pt2 = (int(element["coords_2"][0] * scale_factor) + w1, int(element["coords_2"][1] * scale_factor))
        cv2.circle(vis, pt1, 4, (0, 255, 0), -1)
        cv2.circle(vis, pt2, 4, (0, 255, 0), -1)
        if abs(element['confidence_1'] - element['confidence_2']) > confidenceMean:
            cv2.line(vis, pt1, pt2, (0, 0, 255), 1) #the more unsure you are -> the greater the difference
        else:
            cv2.line(vis, pt1, pt2, (255, 0, 0), 1) # the more sure you are -> the smaller the difference
    ...
    plt.title("Matched Stars & Triplets:")
    plt.axis("off")
    plt.tight_layout()
    plt.show()
